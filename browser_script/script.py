...
def gen_site_globals():
    ...
    # site keyboard
    keyboard = DRIVER.find_element(By.CSS_SELECTOR, "div[class^='Keyboard-module_keyboard']")
    keyboard_rows = keyboard.find_elements(By.CSS_SELECTOR, "div[class^='Keyboard-module_row']")
    ...
    # input board tiles
    board = DRIVER.find_element(By.CSS_SELECTOR, "div[class^='Board-module_board']")
    board_rows = board.find_elements(By.CSS_SELECTOR, "div[class^='Row-module_row']")

    board_rows_tiles = []

    for i in range(6):
        curr_board_row = board_rows[i]
        curr_board_tiles = curr_board_row.find_elements(By.CSS_SELECTOR, "div[class^='Tile-module_tile']")
...
def connect_site():
    ...
    close_popup = DRIVER.find_element(By.CSS_SELECTOR, "div[class^='Modal-module_closeIcon']")
    close_popup.click()
...
def get_feedback(try_num):
    ...
    for i in range(5):
        ...
        curr_tile_state = curr_tile.get_attribute("data-state")

        if curr_tile_state == "correct":
            feedback_map["green"][i] = curr_tile.text
        elif curr_tile_state == "present":
            feedback_map["yellow"][i] = curr_tile.text
        elif curr_tile_state == "absent":
            maybe_greys.add(curr_tile.text)
...
def word_not_in_list(try_num):
    ...
    return last_tile.get_attribute("data-state") == "tbd"
...
def game_won(try_num):
    ...
    for i in range(5):
        curr_tile = recent_board_row[i]
        correct_so_far = correct_so_far and curr_tile.get_attribute("data-state") == "correct"
